...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (this: HTMLAnchorElement, e: Event) {
            ...
            if (!targetId || targetId === '#' || targetId === '#!' || !targetId.startsWith('#')) return;
    ...
    if (notifBell && notifDrop && !window.location.pathname.includes('dashboard.php') && !notifBell.getAttribute('onclick')) {
        ...
        document.addEventListener('click', (e) => {
            if (!notifDrop.contains(e.target as Node) && e.target !== notifBell) {
    ...
    if (isDashboardOrRegister && nuOverlay && !localStorage.getItem('ix_new_user_popup_seen')) {
        ...
        if (rawNu) {
            try { nuData = Object.assign(nuData, JSON.parse(rawNu)); } catch(e) {}
        }
        if (nuData.active) {
            setTimeout(() => {
                ...
                const cta = document.getElementById('appNuCta') as HTMLAnchorElement;
                if (icon) if (icon) icon.textContent = nuData.icon || "IX";
                if (title) title.textContent = nuData.title;
                if (body) body.textContent = nuData.body;
                if (cta) {
    ...
    (window as any).dismissNewUserPopup = function() {
        if (nuOverlay) {
    ...
    async function applyFeatureFlags() {
        ...
        try {
            ...
            if (stored) {
                try { flags = Object.assign(flags, JSON.parse(stored)); } catch(e) {}
            }
        }
    ...
    async function applySiteContent() {
        ...
        try {
            ...
            if (stored) {
                try { siteContent = Object.assign(siteContent, JSON.parse(stored)); } catch(e) {}
            }
        }
